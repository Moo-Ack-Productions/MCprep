...
class MCPREP_OT_prep_materials(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not mat_list:
            if not self.skipUsage:
                self.report(
                    {'ERROR'}, "No materials found on selected objects")
            return {'CANCELLED'}
        ...
        for mat in mat_list:
            if not mat:
                conf.log("During prep, found null material:" +
                         str(mat), vv_only=True)
                continue
            elif mat.library:
                count_lib_skipped += 1
                continue
            ...
            if engine == 'BLENDER_RENDER' or engine == 'BLENDER_GAME':
                ...
                res = generate.matprep_cycles(
                    mat, passes, self.useReflections, self.usePrincipledShader, self.makeSolid, self.packFormat)
                if res == 0:
                    count += 1
            else:
                self.report(
                    {'ERROR'}, "Only Blender Internal, Cycles, or Eevee supported")
                return {'CANCELLED'}
        ...
        if self.combineMaterials is True:
            bpy.ops.mcprep.combine_materials(
                selection_only=True, skipUsage=True)
        ...
        if self.skipUsage is True:
            ...
            self.report(
                {"ERROR"}, "Nothing modified, be sure you selected objects with existing materials!")
...
class MCPREP_OT_swap_texture_pack(bpy.types.Operator, ImportHelper):
    ...
    def preprocess_material(self, material):
        ...
        if material.name == "grass_block_side_overlay":
            ...
            conf.log(
                "Renamed material: grass_block_side_overlay to grass_block_side")
...
class MCPREP_OT_combine_materials(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if bpy.app.version < (2, 78):
            for ob in bpy.data.objects:
                for sl in ob.material_slots:
                    ...
                    sl.material = bpy.data.materials[name_cat[util.nameGeneralize(
                        sl.material.name)][0]]
        ...
        postcount = len(
            ["x" for x in getMaterials(self, context) if x.users > 0])
        self.report({"INFO"},
...
class MCPREP_OT_combine_images(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.selection_only == True and len(context.selected_objects) == 0:
            self.report(
                {'ERROR'}, "Either turn selection only off or select objects with materials/images")
            return {'CANCELLED'}
        ...
        if self.selection_only == True:
            self.report(
                {'ERROR'}, "Combine images does not yet work for selection only, retry with option disabled")
            return {'CANCELLED'}
            ...
            for ob in bpy.data.objects:
                for sl in ob.material_slots:
                    if sl is None or sl.material is None or sl.material not in data:
                        continue  # selection only
                    sl.material = data[name_cat[util.nameGeneralize(
                        sl.material.name)][0]]
            # doesn't remove old textures, but gets it to zero users
        ...
        for f in ob.data.polygons:
            ...
            for i in f.loop_indices:
                if not uv.data[l.index].select and self.selected_only is True:
                    continue
                l = ob.data.loops[i]
                uv.data[l.index].uv[0] = uv.data[l.index].uv[0] * \
                    (1-factor)+x/n*(factor)
                uv.data[l.index].uv[1] = uv.data[l.index].uv[1] * \
                    (1-factor)+y/n*(factor)
                modified = True
        if not modified:
...
class MCPREP_OT_select_alpha_faces(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if ob is None:
            ...
            self.report(
                {"ERROR"}, "Active object has no materials to check image for.")
            return {"CANCELLED"}
