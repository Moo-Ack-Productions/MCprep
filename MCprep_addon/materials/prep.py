...
class McprepPrepMaterial():
	"""Base class for Prep Material
	...
	def pack_formats(self, context):
		...
		return [(i.name.lower(), i.name.capitalize(), i.__doc__) for i in generate.PackFormat]

	animateTextures: BoolProperty(
		...
		default=False)
	autoFindMissingTextures: BoolProperty(
	...
	def draw_before(self, context: Context) -> None:
		pass
	
	def draw_after(self, context: Context) -> None:
		pass

	def draw(self, context: Context) -> None:
		self.draw_before(context)
		# For swap texture if prepMaterials is on
		if hasattr(self, "prepMaterials"):
			if not getattr(self, "prepMaterials"):
				return 
		row = self.layout.row()
		col = row.column()
	...
	def prep_mat(self, context: Context):
		"""A method for prepping material
...
class MCPREP_OT_prep_materials(McprepPrepMaterial, Operator):
	...
	def execute(self, context):
		result = self.prep_mat(context)
		if result:
			return {'CANCELLED'}
		return {'FINISHED'}


class MCPREP_OT_materials_help(Operator):
	...
	help_num: IntProperty(
		default=0,
		options={'HIDDEN'})
	suppress_help: BoolProperty(
...
class MCPREP_OT_swap_texture_pack(
	McprepPrepMaterial, Operator, ImportHelper):
...
class MCPREP_OT_load_material(McprepPrepMaterial, Operator):
	...
	skipUsage: BoolProperty(default=False, options={'HIDDEN'})
