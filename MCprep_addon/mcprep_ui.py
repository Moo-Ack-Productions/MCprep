...
class McprepPreference(bpy.types.AddonPreferences):
    ...
    def draw(self, context):
        ...
        if self.preferences_tab == "settings":
            ...
            col = split.column()
            p = col.operator("mcprep.openfolder",
                             text="Open texture pack folder")
            ...
            col = split.column()
            col.operator("mcprep.mob_install_menu",
                         text="Install file for mob spawning")
            col = split.column()
            ...
            col = split.column()
            col.operator("mcprep.add_skin",
                         text="Install skin file for swapping")
            col = split.column()
...
class MCPREP_PT_skins(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not scn_props.show_settings_skin:
            ...
            b_subrow.operator("mcprep.skin_path_reset",
                              icon=LOAD_FACTORY, text="")
            ...
            if context.mode == "OBJECT" and skinname:
                ...
                if not scn_props.mob_list:
                    row.enabled = False
                    row.operator("mcprep.spawn_with_skin",
                                 text="Reload mobs below")
                elif not conf.skin_list:
                    row.enabled = False
                    row.operator("mcprep.spawn_with_skin",
                                 text="Reload skins above")
                else:
